class UserController < ApplicationController
    ...
    def new
        ...
        if user == nil
            user = User.new({name: params[:name], email: params[:email], uid: params[:uid], image: params[:image]}).save
            Setting.new({user_id: user.id, location_share: false, ferries: false, tolls: false, highways: false}).save
        end
